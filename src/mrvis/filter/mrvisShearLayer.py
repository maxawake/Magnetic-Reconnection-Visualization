...
@smproxy.filter(label="MRVIS Shear Layer")
@smproperty.input(name="Input", port_index=0)
@smdomain.datatype(dataTypes=["vtkDataSet"])
class mrvisShearLayer(VTKPythonAlgorithmBase):
    def __init__(self):
        self._array_field = 0
    ...
    def RequestData(self, request, inInfo, outInfo):
        ...
        Jacobian = np.array([[dudx, dudy, dudz], [dvdx, dvdy, dvdz], [dwdx, dwdy, dwdz]])
        ...
        result = -(
            eigenvals[:, 0] * eigenvals[:, 1] + eigenvals[:, 0] * eigenvals[:, 2] + eigenvals[:, 1] * eigenvals[:, 2]
        )
        # result = np.sqrt(
        #     (
