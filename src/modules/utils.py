...
def save_as_vtk(vectorfield, savepath="./", name="output", factor=3, show=False):
    print()
    data_new = vectorfield[:, ::factor, ::factor, ::factor]
    field = VectorField(data_new, np.array(data_new[0].shape), [1, 1, 1], name)
    if show:
        field.render()
    field.save(savepath + name)
